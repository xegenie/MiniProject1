DROP TABLE IF EXISTS `rental list`;
...
CREATE TABLE `board` (
	...
	`type`	VARCHAR(30)	NOT NULL	COMMENT '타입(게시판, 공지사항)'
);

DROP TABLE IF EXISTS `hope book`;

CREATE TABLE `hope book` (
	`id`	VARCHAR(100)	NOT NULL	COMMENT '회원ID',
...
DROP TABLE IF EXISTS `book stock`;

CREATE TABLE `book stock` (
	`stock_id`	INT	NOT NULL	COMMENT '재고 ID',
	`book_id`	INT	NOT NULL	COMMENT '도서 ID',
	`status`	VARCHAR(30)	NOT NULL	DEFAULT 대출 가능	COMMENT '상태(대출 가능, 대출 불가, 예약 중)'
);

DROP TABLE IF EXISTS `wish list`;
...
CREATE TABLE `files` (
	...
	`type`	VARCHAR(30)	NOT NULL	COMMENT '부모테이블(board, book)'
);
